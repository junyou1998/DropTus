...
onMounted(async () => {
  ...
  try {
    ...
    console.warn("無法監聽視窗聚焦狀態（非 Tauri 執行期）：", e);
  }
});
...
async function startUpload(file: File) {
  ...
  try {
    ...
    currentUpload.value.error = err.message || "上傳失敗";
...
<template>
  <div class="w-full h-screen bg-transparent p-3 flex flex-col select-none">
    ...
    >
      <div data-tauri-drag-region class="pointer-events-none flex items-center gap-2">
        ...
        <div data-tauri-drag-region>
          ...
          <p class="text-[9px] text-neutral-400 font-medium">
            {{ activeProfile ? activeProfile.name : "未連線" }} • {{ activeTemplate ? activeTemplate.name : "無模板" }}
          </p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-1.5">
        ...
        <button
          ...
          :title="isPinned ? '已釘住視窗（失焦不隱藏）' : '未釘住視窗（失焦自動隱藏）'"
        >
          <Pin class="w-3 h-3" />
        </button>
    ...
    <div class="flex-1 my-3 flex flex-col justify-center gap-3">
      ...
      >
        <UploadCloud
          ...
          :class="isDragging ? 'text-indigo-500 scale-115' : 'text-neutral-400'"
        />
    ...
    <div class="h-44 border-t border-neutral-100 dark:border-neutral-800/60 pt-2 flex flex-col">
      <div class="text-[10px] font-bold text-neutral-400 uppercase mb-2">最近上傳紀錄</div>
